...
run_case() {
    ...
    for server_id in "${!SERVER_IPS[@]}"; do
        start_server "$server_id" "${SERVER_IPS[$server_id]}" "$server_count" "$max_inflight" "$enable_priority" "$config_remote"
        sleep 1
    done
    ...
    for client_vm_idx in "${!CLIENT_IPS[@]}"; do
        start_client "$client_id" "${CLIENT_IPS[$client_vm_idx]}" "$server_count" "$max_inflight" "$enable_priority" "$config_remote"

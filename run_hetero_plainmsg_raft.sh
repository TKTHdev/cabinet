#!/bin/bash
# ================================================================
# HETEROGENEOUS PLAIN-MSG EVALUATION RUNNER (RAFT VARIANT)
# Runs the 5-node Cabinet cluster across all experiment families.
# This variant enforces ENABLE_PRIORITY=false.
# ================================================================
...
# WoC-specific keys removed. ENABLE_PRIORITY=false for raft runs.
BASE_ENV=(
    "NUM_SERVERS=5"
    "NUM_CLIENTS=2"
    # Raft variant uses a higher threshold than the Cabinet baseline.
    "THRESHOLD=2"
    "OPS=0"
    "EVAL_TYPE=0"
    "BATCHSIZE=1"
...
Selectors (default: all):
  ...
  eval4          Network delay: D1 uniform sweep + D4 burst
    eval4s         Network delay with sampled client timeseries
...
# ================================================================
# EVAL 2: client max inflight sweep
# ================================================================
if [[ "$EVAL_ONLY" == "all" || "$EVAL_ONLY" == "eval2" ]]; then
    echo "── EVAL 2: Client max inflight sweep ───────────────────────────"
    for max_inflight in 1 2 3 4 5 8 10 15 20 25 30 35 40 ; do
        BASE_ENV=(
            "NUM_SERVERS=5" "NUM_CLIENTS=2" "THRESHOLD=2" "OPS=0"
            "EVAL_TYPE=0" "BATCHSIZE=1" "MSG_SIZE=512" "MODE=1"
...
# ================================================================
# EVAL 4s: Network delay with sampled client timeseries
# ================================================================
if [[ "$EVAL_ONLY" == "all" || "$EVAL_ONLY" == "eval4s" ]]; then
    ...
    echo ""
    echo "── D1: Uniform delays (MAX_INFLIGHT=1) ──────────────────────────"
    ...
    for delay_ms in "${D1_DELAYS[@]}"; do
        ...
        BASE_ENV=(
            ...
            "BATCH_MODE=single" "BATCH_COMPOSITION=object-specific"
            "LOG_LEVEL=debug" "ENABLE_PRIORITY=false" "RATIO_STEP=0.001"
            "MAX_INFLIGHT=1" "ENABLE_TIMESERIES=true"
        )
    ...
    echo "── D4: Bursting (MAX_INFLIGHT=1, 15s calm / 10s spike) ──────────"
    BASE_ENV=(
        "NUM_SERVERS=5" "NUM_CLIENTS=2" "THRESHOLD=2" "OPS=0"
        "EVAL_TYPE=0" "BATCHSIZE=1" "MSG_SIZE=512" "MODE=1"
        "HOT_RATIO=0" "INDEP_RATIO=90" "COMMON_RATIO=10"
